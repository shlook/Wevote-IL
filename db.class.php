<?
  ...
  class DB
  {
    ...
    function DB($base, $server, $user, $pass)
    {
      ...
      $con = mysql_connect($server, $user, $pass) or die('Server connecion not possible.');
	...
	# Set timezone
	  mysql_query("SET time_zone = '+2:00'", $con);
      mysql_select_db($base)               or die('Database connecion not possible.');
    }
	...
	{
		$msg = htmlspecialchars(date('d.m.Y h:i:s')." Failed Query: {$query}   SQL Error: {$error}\n\r");
		error_log($msg, 3, "errlog/sql_error.log");		
	}
    function debugAndDie($query)
    {
	 ...
	 // $globalDebug = true;
	  ...
	  {
		$this->sql_failure_handler($query, mysql_error());
		die("Requested page is temporarily unavailable, please try again later.");
	  }
    }
